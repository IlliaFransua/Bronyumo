...
def make_instruction(path, file_name):
    text_en = f"""
# 🇺🇸 Converting the markup of the footer "{file_name}" into code

[Watch the video guide on working with tasks in Bronyumo](https://google.com)

## Task Description
You need to implement the site footer (footer) **"{file_name}"** according to the provided design. The design is available in the `Resources` section at the bottom of this text.

## Expected Result
All the code for this task is stored in `Resources/{file_name}`. The site footer (footer) should display correctly on the page, match the design, and use auto-layout (if specified in the design).
...
- **Folder for Footer Resources:** `Resources/{file_name}`
    """

    text_ua = f"""
# 🇺🇦 Верстка нижнього колонтитула сайту (футера) "{file_name}"

[Дивись відео-гайд щодо роботи з тасками в Bronyumo](https://google.com)

## Опис завдання
Необхідно зверстати нижній колонтитул сайту (футер) **"{file_name}"** за наданим макетом. Макет доступний у розділі `Ресурси` внизу цього тексту.

## Очікуваний результат
Весь код цього завдання зберігається в `Resources/{file_name}`. Нижній колонтитул сайту (футер) повинен коректно відображатися на сторінці, відповідати макету та використовувати auto-layout (якщо це передбачено макетом).
...
- **Папка для ресурсів нижнього колонтитула сайту (футера):** `Resources/{file_name}`
    """
...
if __name__ == "__main__":
    print_directories_with_prefix('Resources', 'footer')