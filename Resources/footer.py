...
def make_instruction(path, file_name):
    text = f"""
# Верстка нижнего колонтитула сайта (футера) "{file_name}"

[Смотри видео-гайд по работе с тасками в Bronyumo](https://google.com)

## Описание задачи
Необходимо сверстать нижний колонтитул сайта (футер) **"{file_name}"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/{file_name}`. Нижний колонтитул сайта (футер) должен корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
## Ресурсы
- **Макет нижнего колонтитула сайта (футера):** [ссылка на макет, ищи Layer с названием:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ```bash
    {file_name}
    ```
- **Рабочая ветка:** `feauture/{file_name}` от `dev`
- **Папка для ресурсов нижнего колонтитула сайта (футера):** `Resources/{file_name}`
    """
