# 🇺🇦 Створити обробник сторінки "render_reservations_panel"

## Опис завдання
Необхідно реалізувати обробник сторінки **"render_reservations_panel"** відповідно до наданого макету. Макет можна переглянути за посиланням:
...
- **Робоча гілка:**  
  Перейдіть на гілку `feature/apps/render_reservations_panel`, створену від `dev`.
- **Завершення завдання:**  
...
- [Макет сторінки, шукай елемент з назвою:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)  
    ...
    ```
- **Робоча гілка:**  
  `feature/apps/render_reservations_panel` від `dev`
---
# 🇺🇸 Create a handler for the page "render_reservations_panel"

## Task Description
You need to implement a handler for the page **"render_reservations_panel"** according to the provided layout. The layout can be viewed at the following link:
...
- **Working Branch:**  
  Switch to the branch `feature/apps/render_reservations_panel`, which was created from `dev`.
- **Task Completion:**  
...
- [Page layout, look for the element named:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)  
    ...
    ```
- **Working Branch:**  
  `feature/apps/render_reservations_panel` from `dev`