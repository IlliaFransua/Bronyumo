# 🇺🇦 Створити API-обробник "upload_and_preserve_bookings"

## Опис завдання
Необхідно реалізувати API-обробник **"upload_and_preserve_bookings"** відповідно до коментарів наданих у макеті. Макет можна переглянути за посиланням:
...
- **Робоча гілка:**  
  Перейдіть на гілку `feature/apps/upload_and_preserve_bookings`, створену від `dev`.
- **Завершення завдання:**  
...
- [У макеті шукайте коментарі, в яких згадується:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)  
    ...
    ```
- **Робоча гілка:**  
  `feature/apps/upload_and_preserve_bookings` від `dev`
---
# 🇺🇸 Create an API handler "upload_and_preserve_bookings"

## Task Description
You need to implement the API handler **"upload_and_preserve_bookings"** according to the comments provided in the layout. The layout can be viewed at the following link:
...
- **Working Branch:**  
  Switch to the branch `feature/apps/upload_and_preserve_bookings`, which was created from `dev`.
- **Task Completion:**  
...
- [In the layout, look for comments that mention:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)  
    ...
    ```
- **Working Branch:**  
  `feature/apps/upload_and_preserve_bookings` from `dev`