# 🇺🇦 Створити API-обробник "share_booking"

## Опис завдання
Необхідно реалізувати API-обробник **"share_booking"** відповідно до коментарів наданих у макеті. Макет можна переглянути за посиланням:
...
- **Робоча гілка:**  
  Перейдіть на гілку `feature/apps/share_booking`, створену від `dev`.
- **Завершення завдання:**  
...
- [У макеті шукайте коментарі, в яких згадується:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)  
    ...
    ```
- **Робоча гілка:**  
  `feature/apps/share_booking` від `dev`
---
# 🇺🇸 Create an API handler "share_booking"

## Task Description
You need to implement the API handler **"share_booking"** according to the comments provided in the layout. The layout can be viewed at the following link:
...
- **Working Branch:**  
  Switch to the branch `feature/apps/share_booking`, which was created from `dev`.
- **Task Completion:**  
...
- [In the layout, look for comments that mention:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)  
    ...
    ```
- **Working Branch:**  
  `feature/apps/share_booking` from `dev`