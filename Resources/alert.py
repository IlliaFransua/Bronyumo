...
def make_instruction(path, file_name):
    text_en = f"""
# 🇺🇸 Converting the markup of the popup notification "{file_name}" into code

[Watch the video guide on working with tasks in Bronyumo](https://google.com)

## Task Description
You need to implement the popup notification **"{file_name}"** according to the provided design. The design is available in the `Resources` section at the bottom of this text.

## Expected Result
All the code for this task is stored in `Resources/{file_name}`. The popup notification should display correctly on the page, match the design, and use auto-layout (if specified in the design).
...
- **Folder for Popup Notification Resources:** `Resources/{file_name}`
    """

    text_ua = f"""
# 🇺🇦 Верстка спливаючого сповіщення "{file_name}"

[Дивись відео-гайд щодо роботи з тасками в Bronyumo](https://google.com)

## Опис завдання
Необхідно зверстати спливаюче сповіщення **"{file_name}"** за наданим макетом. Макет доступний у розділі `Ресурси` внизу цього тексту.

## Очікуваний результат
Весь код цього завдання зберігається в `Resources/{file_name}`. Спливаюче сповіщення повинно коректно відображатися на сторінці, відповідати макету та використовувати auto-layout (якщо це передбачено макетом).
...
- **Папка для ресурсів спливаючого сповіщення:** `Resources/{file_name}`
    """
...
if __name__ == "__main__":
    print_directories_with_prefix('Resources', 'alert')