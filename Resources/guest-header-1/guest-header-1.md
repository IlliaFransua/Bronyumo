...
## Описание задачи
Необходимо сверстать шапку **"guest-header-1"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/guest-header-1`. Шапка должна корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
- **Рабочая ветка:**  
  Переключитесь на ветку `feauture/guest-header-1`, которая создана от `dev`.
...
- **Макет шапки:** [ссылка на макет, ищи Layer с названием:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    guest-header-1
    ```
- **Рабочая ветка:** `feauture/guest-header-1` от `dev`
- **Папка для ресурсов шапки:** `Resources/guest-header-1`
