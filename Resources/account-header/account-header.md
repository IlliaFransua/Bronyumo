...
## Описание задачи
Необходимо сверстать шапку **"account-header"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/account-header`. Шапка должна корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
- **Рабочая ветка:**  
  Переключитесь на ветку `feauture/account-header`, которая создана от `dev`.
...
- **Макет шапки:** [ссылка на макет, ищи Layer с названием:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    account-header
    ```
- **Рабочая ветка:** `feauture/account-header` от `dev`
- **Папка для ресурсов шапки:** `Resources/account-header`
