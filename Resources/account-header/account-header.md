# 🇺🇸 Converting the markup of the header "account-header" into code

[Watch the video guide on working with tasks in Bronyumo](https://google.com)

## Task Description
You need to implement the header **"account-header"** according to the provided design. The design is available in the `Resources` section at the bottom of this text.

## Expected Result
All the code for this task is stored in `Resources/account-header`. The header should display correctly on the page, match the design, and use auto-layout (if specified in the design).
...
- **Working Branch:**  
  Switch to the branch `feauture/account-header`, which is created from `dev`.
...
- **Header Design:** [link to the design, look for the Layer named:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    account-header
    ```
- **Working Branch:** `feauture/account-header` from `dev`
- **Folder for Header Resources:** `Resources/account-header`
---
# 🇺🇦 Верстка шапки "account-header"

[Дивись відео-гайд щодо роботи з тасками в Bronyumo](https://google.com)

## Опис завдання
Необхідно зверстати шапку **"account-header"** за наданим макетом. Макет доступний у розділі `Ресурси` внизу цього тексту.

## Очікуваний результат
Весь код цього завдання зберігається в `Resources/account-header`. Шапка повинна коректно відображатися на сторінці, відповідати макету та використовувати auto-layout (якщо це передбачено макетом).
...
- **Робоча гілка:**  
  Переключіться на гілку `feauture/account-header`, яка створена від `dev`.
...
- **Макет шапки:** [посилання на макет, шукай Layer з назвою:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    account-header
    ```
- **Робоча гілка:** `feauture/account-header` від `dev`
- **Папка для ресурсів шапки:** `Resources/account-header`