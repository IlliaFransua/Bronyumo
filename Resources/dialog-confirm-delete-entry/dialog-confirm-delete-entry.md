...
# Верстка диалогового окна "dialog-confirm-delete-entry"

[Смотри видео-гайд по работе с тасками в Bronyumo](https://google.com)

## Описание задачи
Необходимо сверстать диалоговое окно **"dialog-confirm-delete-entry"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/dialog-confirm-delete-entry`. Диалоговое окно должно корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
- **Рабочая ветка:**  
  Переключитесь на ветку `feauture/dialog-confirm-delete-entry`, которая создана от `dev`.
...
- **Макет диалогового окна:** [ссылка на макет, ищи Layer с названием:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    ```
- **Рабочая ветка:** `feauture/dialog-confirm-delete-entry` от `dev`
- **Папка для ресурсов диалогового окна:** `Resources/dialog-confirm-delete-entry`
