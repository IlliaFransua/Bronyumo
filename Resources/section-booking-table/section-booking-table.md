...
# Верстка секции "section-booking-table"

[Смотри видео-гайд по работе с тасками в Bronyumo](https://google.com)

## Описание задачи
Необходимо сверстать секцию **"section-booking-table"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/section-booking-table`. Секция должна корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
- **Рабочая ветка:**  
  Переключитесь на ветку `feauture/section-booking-table`, которая создана от `dev`.
...
- **Макет секции:** [ссылка на макет, ищи Layer с названием:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    ```
- **Рабочая ветка:** `feauture/section-booking-table` от `dev`
- **Папка для ресурсов секции:** `Resources/section-booking-table`
