# 🇺🇸 Converting the markup of the popup notification "alert-success-message" into code

[Watch the video guide on working with tasks in Bronyumo](https://google.com)

## Task Description
You need to implement the popup notification **"alert-success-message"** according to the provided design. The design is available in the `Resources` section at the bottom of this text.

## Expected Result
All the code for this task is stored in `Resources/alert-success-message`. The popup notification should display correctly on the page, match the design, and use auto-layout (if specified in the design).
...
- **Working Branch:**  
  Switch to the branch `feauture/alert-success-message`, which is created from `dev`.
...
- **Popup Notification Design:** [link to the design, look for the Layer named:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    ```
- **Working Branch:** `feauture/alert-success-message` from `dev`
- **Folder for Popup Notification Resources:** `Resources/alert-success-message`
---
# 🇺🇦 Верстка спливаючого сповіщення "alert-success-message"

[Дивись відео-гайд щодо роботи з тасками в Bronyumo](https://google.com)

## Опис завдання
Необхідно зверстати спливаюче сповіщення **"alert-success-message"** за наданим макетом. Макет доступний у розділі `Ресурси` внизу цього тексту.

## Очікуваний результат
Весь код цього завдання зберігається в `Resources/alert-success-message`. Спливаюче сповіщення повинно коректно відображатися на сторінці, відповідати макету та використовувати auto-layout (якщо це передбачено макетом).
...
- **Робоча гілка:**  
  Переключіться на гілку `feauture/alert-success-message`, яка створена від `dev`.
...
- **Макет спливаючого сповіщення:** [посилання на макет, шукай Layer з назвою:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    alert-success-message
    ```
- **Робоча гілка:** `feauture/alert-success-message` від `dev`
- **Папка для ресурсів спливаючого сповіщення:** `Resources/alert-success-message`