...
# Верстка всплывающего оповещения "alert-error-message"

[Смотри видео-гайд по работе с тасками в Bronyumo](https://google.com)

## Описание задачи
Необходимо сверстать всплывающее оповещение **"alert-error-message"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/alert-error-message`. Всплывающее оповещение должен корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
- **Рабочая ветка:**  
  Переключитесь на ветку `feauture/alert-error-message`, которая создана от `dev`.
...
- **Макет всплывающего оповещения:** [ссылка на макет, ищи Layer с названием:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    alert-error-message
    ```
- **Рабочая ветка:** `feauture/alert-error-message` от `dev`
- **Папка для ресурсов всплывающего оповещения:** `Resources/alert-error-message`
