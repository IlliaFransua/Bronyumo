# 🇺🇸 Converting the markup of the header "guest-header-2" into code

[Watch the video guide on working with tasks in Bronyumo](https://google.com)

## Task Description
You need to implement the header **"guest-header-2"** according to the provided design. The design is available in the `Resources` section at the bottom of this text.

## Expected Result
All the code for this task is stored in `Resources/guest-header-2`. The header should display correctly on the page, match the design, and use auto-layout (if specified in the design).
...
- **Working Branch:**  
  Switch to the branch `feauture/guest-header-2`, which is created from `dev`.
...
- **Header Design:** [link to the design, look for the Layer named:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    guest-header-2
    ```
- **Working Branch:** `feauture/guest-header-2` from `dev`
- **Folder for Header Resources:** `Resources/guest-header-2`
---
...
## Опис завдання
Необхідно зверстати шапку **"guest-header-2"** за наданим макетом. Макет доступний у розділі `Ресурси` внизу цього тексту.

## Очікуваний результат
Весь код цього завдання зберігається в `Resources/guest-header-2`. Шапка повинна коректно відображатися на сторінці, відповідати макету та використовувати auto-layout (якщо це передбачено макетом).
...
- **Робоча гілка:**  
  Переключіться на гілку `feauture/guest-header-2`, яка створена від `dev`.
...
- **Макет шапки:** [посилання на макет, шукай Layer з назвою:](https://www.figma.com/design/XMX1W4mwttgUy8L0a4kzQe/Bronyumo.ua-(special-task-mockup)?node-id=0-1&t=wnXX2PPEWtIi002e-1)
    ...
    guest-header-2
    ```
- **Робоча гілка:** `feauture/guest-header-2` від `dev`
- **Папка для ресурсів шапки:** `Resources/guest-header-2`