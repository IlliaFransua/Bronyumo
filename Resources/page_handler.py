...
def make_instruction(file_name):
    text_en = f"""
# 🇺🇸 Create a handler for the page "{file_name}"

## Task Description
You need to implement a handler for the page **"{file_name}"** according to the provided layout. The layout can be viewed at the following link:
...
- **Working Branch:**  
  `feature/apps/{file_name}` from `dev`
    ...
    text_ua = f"""
# 🇺🇦 Створити обробник сторінки "{file_name}"

## Опис завдання
Необхідно реалізувати обробник сторінки **"{file_name}"** відповідно до наданого макету. Макет можна переглянути за посиланням:
...
if __name__ == "__main__":
    file_names = ["render_home_page", "render_entrepreneur_page", "render_client_panel", "render_entrepreneur_panel", "edit_available_objects", "render_reservations_panel"]
