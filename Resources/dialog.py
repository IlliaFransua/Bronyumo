...
def make_instruction(path, file_name):
    text = f"""
# Верстка диалогового окна "{file_name}"

[Смотри видео-гайд по работе с тасками в Bronyumo](https://google.com)

## Описание задачи
Необходимо сверстать диалоговое окно **"{file_name}"** по предоставленному макету. Макет доступен в разделе `Ресурсы` в самом низу этого текста.

## Ожидаемый результат
Весь код этой задачи хранится в `Resources/{file_name}`. Диалоговое окно должно корректно отображаться на странице, соответствовать макету и использовать auto-layout (если таковой предусмотрен в макете).
...
- **Папка для ресурсов диалогового окна:** `Resources/{file_name}`
    """
